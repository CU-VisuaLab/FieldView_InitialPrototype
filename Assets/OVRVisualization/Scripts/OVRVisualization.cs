...
public class OVRVisualization : MonoBehaviour
{
    // Singleton instance
    public static OVRVisualization instance { get; private set; }
    ...
    [Serializable]
    public class VisualizationShowEvent : UnityEvent { }

    [Serializable]
    public class VisualizationHideEvent : UnityEvent { }
    [Header("Events")]
    [SerializeField]
    ...
    static public OVRCameraRig cameraRig 
    {
        get
        {
            Debug.Log("GETTER " + GameObject.Find("OVRCameraRig").GetComponent<OVRCameraRig>().centerEyeAnchor);
...
#region StartUpFunctions 
    void Awake()
    {
        Debug.Log(string.Format("OVRVisualization Awake", 0));

        playerLayer = GetLayerOrReportError("Player");

        if (instance != null)
        {
            Debug.LogError("Existing OVRVisualization");
    ...
    void Update()
    {
        ...
        Debug.Log("UPDATE: " + activeTransform);
        UpdateFramerate();
